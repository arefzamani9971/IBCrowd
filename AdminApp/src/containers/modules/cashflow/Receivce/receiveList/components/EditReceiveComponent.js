...
import UpdateReceiveService from "../services/UpdateReceiveService";
class EditReceiveComponent extends React.Component {
    constructor(props) {
        super(props);
        ...
        this.state = {
            ...
            getMainMarketTitle: '',
            toBankDepositId: null,
            cashFlowCategoryTitle: null,
            mainMarketId: null,
            engType: null,
            id: 0,
    ...
    componentDidMount() {
        ...
        GetAllCashFlowCategoryService.getreceivebyidMethod({
            "entity": this.props.history.location.state.stateParams.id,
        }, (response) => {
            let { amount, trakingNumber, created, dueDateJalali, fromPartyFullName, dueDate, description, mainMarketId, engType, fromPartyId, toBankDepositId, toBankDepositTitle, cashFlowCategoryTitle, id } = response.result; // Destructuring assignment
            this.setState({
                ...
                GetChequeBookServices.searchBankDepositMethod(reportFilter, (response) => {
                    for (let i = 0; i < response.result.length; i++) {
                        if (this.state.toBankDepositId == response.result[i].id) {
                            this.setState({
        ...
        return (
            <React.Fragment>
                ...
                <Form
                    {...this.props}
                    {...this.state}
                    service={UpdateReceiveService.updatereceiveMethod}
                    entity={
                        {
                            id: this.state.id,
                            fromPartyId: this.state.partyId,
                ...
                >
                    <br />
                        ...
                        <Grid container spacing={8} className="no-margin">
                            <Grid item md={3}>
                                <NoDataDatePicker isNull={true} selectedDate={this.state.date} label="از تاریخ" handleOnChange={(value) => this.handleDate(value, 'date')} />
                            </Grid>
                        </Grid>
                        <Grid container spacing={8} className="no-margin">
                            <Grid item md={3}>
                        ...
                        <Grid container spacing={8} className="no-margin">
                            <Grid item md={6}>
                                <Input label="توضیحات" handleChange={(e) => this.handleChange(e, 'comment')} value={this.state.comment} />
                            </Grid>
                        </Grid>
                </Form>
            </React.Fragment>
...
export default EditReceiveComponent;