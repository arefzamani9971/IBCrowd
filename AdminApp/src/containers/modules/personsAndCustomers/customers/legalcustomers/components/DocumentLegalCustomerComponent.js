...
class DocumentLegalCustomerComponent extends React.Component {
  constructor(props) {
    super(props);
    ...
    if (this.props.location.state && !this.props.location.state.customeBackInfo) {
      this.setState({
        ...
        GetManageCustomerRecordsServices.getAllAttachmentCategoryMethod({ partyId: this.state.partyId }, (res) => {
          ...
          this.setState({
            getAllAttachmentCategory: res.result.filter(res => res.partyType==2)
          })
        })
      })
...
DocumentLegalCustomerComponent.propTypes = {
  classes: PropTypes.object.isRequired,
};

export default withStyles(styles)(DocumentLegalCustomerComponent);