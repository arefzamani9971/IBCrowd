...
import Columns from '../constants/GetNotEnteredOrderColumn';
...
import './GetNotEnteredOrderListComponent.css'
...
class GetNotEnteredOrderList extends React.Component {
      ...
      dataSource: {
        transport: {
          read: function (option) {
            ...
            GetTseOrderBranchService.getAllNotEnteredOrderList(command, function (response) {
   ...
   ${excelAndPdfToolbar}`,
      dataBound: function (e) {
        ...
        if (this.dataSource.data().length > 0) {
          let grid = $("#not-entered-orders").data("kendoGrid");
    ...
    $("#not-entered-orders .excel-report").on("click", function (item) {
      self.getExcelReport();

    });
    $("#not-entered-orders .pdf-report").on("click", function (item) {
      self.getPdfReport();

    });
  };
  getCommand = () => {
    var grid = $("#not-entered-orders").data("kendoGrid");
  ...
  getExcelReport = () => {
    ...
    // alert('excel')
    GetTseOrderBranchService.getExcelExportDownloadExcelGetAllNotEntered(command, 'سفارش وارد نشده');
  ...
  search() {
    $("#not-entered-orders").data("kendoGrid").dataSource.read(this);
    this.setState({
      open: false
    })
    selectedIds=[];
  ...
  handleCloseAssignBranchModal = () => {
    this.setState({ backToWaitingListModal: false, selectedId: null });
  };
  ...
  render() {
    ...
    return (
      <React.Fragment>
        <Paper className={"main-paper-container not-entered-orders"}>
          <Filter search={this.search}
            ...
            <div classPage={"height-search"}>
              <Grid container spacing={8} className="no-margin">
                ...
                <Grid item md={2}>
                  <Input label="تا شماره سریال " type="number" handleChange={(e) => this.handleChange(e, 'toSerialNumber')} value={this.state.toSerialNumber} />
                </Grid>
                <Grid item md={2}>
                  <FormControlLabel
                    style={{ marginTop: 14 }}
          ...
          <div className={"k-rtl " + (this.state.open ? "height-open-grid" : "height-content-grid")}>
            <div id="not-entered-orders" className="height-page"></div>
          </div>
        </Paper>
        <Modal
...
export default GetNotEnteredOrderList;
