...
class GetOrders extends React.Component {
  ...
  getOrderList() {
        ...
        if (this.dataSource.data().length > 0) {
          ...
          items.map((item, index) => {
            ...
            if (index === this.dataSource.data().length - 1 && this.dataSource.data().length > 1) {
              ...
              $("#orders-list tbody tr td span.detail").on("click", function (item) {
    ...
    $("#orders-list .excel-report").on("click", function (item) {
      self.getExcelReport();

    });
    $("#orders-list .pdf-report").on("click", function (item) {
      self.getPdfReport();
  ...
  getExcelReport = () => {
    ...
    // alert('excel')
    GetOrdersService.getExcelExport(command, 'فهرست سفارش ها');
  }

  getPdfReport = () => {
    var command = this.getCommand();
    // alert('pdf')
    GetOrdersService.getPdfExport(command, "فهرست سفارش ها");
  }
