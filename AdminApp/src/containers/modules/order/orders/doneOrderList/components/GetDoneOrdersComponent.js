...
import Columns from '../constants/GetDoneOrdersColumn';
...
import './GetDoneOrdersComponent.css';
...
import GetDoneOrdersService from '../services/GetDoneOrdersService';
import GetOrdersService from '../../ordersList/services/GetOrdersService';

const $ = require("jquery");
let selectedIds = [];

class GetDoneOrders extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      orderState: [{ code: 5, title: "قسمتی از سفارش انجام شده" }, { code: 6, title: "کاملا انجام شده" }
        , { code: 8, title: "قسمتی از سفارش انجام شده و مابقی منقضی شده" }, { code: 10, title: "قسمتی از سفارش انجام شده و مابقی ابطال شده" }
      ],
      orderTypeList: {
        name: "orderType",
        field: "title",
  ...
  getOrderState() {
    GetEnum('GetDoneOrderState', response => DropDownListDataProvider(this, "orderStateList", response))
  ...
  search() {
    ...
    $("#orders-done-list").data("kendoGrid").dataSource.read(this);
    this.setState({
      open: false
    })
    selectedIds = [];
  ...
  getOrderList() {
    ...
    $("#orders-done-list").kendoGrid({
      ...
      dataBound: function (e) {
        ...
        if (this.dataSource.data().length > 0) {
          ...
          let grid = $("#orders-done-list").data("kendoGrid");
          ...
          $("#orders-done-list tbody tr td span.edit").on("click", function (item) {
            var grid = $("#orders-done-list").data("kendoGrid");
          ...
          $("#orders-done-list tbody tr td span.detail").on("click", function (item) {
            ...
            self.props.history.push(
              {
                pathname: '/main/order/orders/getMatchOrders',
                state: {
                  stateParams: {
                    id: dataItem.id,
                    ...
                    serialNumber: dataItem.serialNumber,
                    dateJalali: dataItem.dateJalali
                  }
                }

              })
          });
          $("#orders-done-list tbody tr td span.delete").on("click", function (item) {
            var grid = $("#orders-done-list").data("kendoGrid");
            var row = $(item.target).closest("tr");
            var dataItem = grid.dataItem(row);
          ...
          items.map((item, index) => {
            ...
            if (index === this.dataSource.data().length - 1 && this.dataSource.data().length > 1) {
              $("#orders-done-list .k-grid-footer .k-grid-footer-wrap " +
    ...
    $("#orders-done-list .excel-report").on("click", function (item) {
      self.getExcelReport();

    });
    $("#orders-done-list .pdf-report").on("click", function (item) {
      self.getPdfReport();
  ...
  reRenderGrid() {
    ...
    var self = this;
    GetDoneOrdersService.deleteOrder(command, (res) => {
  ...
  render() {
    ...
    return (
      <React.Fragment>
        <Header {...this.props} />
        <Paper className={"main-paper-container orders-done-list fade-in"}>
          ...
          <div className={"k-rtl " + (this.state.open ? "height-open-grid" : "height-content-grid") + " " + (this.state.isLoading ? 'hidden-item' : 'show-item')}>
            <div id="orders-done-list" className="height-page"></div>
          </div>
        </Paper>
        <Modal
